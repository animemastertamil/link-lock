...
async function onDecrypt() {
    ...
    if (!("b64" in window && "apiVersions" in window)) {
        error("System Error: Critical libraries are missing. The AnimeMaster Tamil decoder cannot function!");
        return;
    }
    ...
    try {
        ...
        error("Invalid Link: That is not a valid URL. Ensure it includes \"https://\"!");
        return;
    }
    ...
    try {
        ...
        error("Link Corrupted: The encrypted AnimeMaster Tamil link is damaged or invalid.");
        return;
    }

    // Check that all required parameters encoded in the URL are present
    if (!("v" in params && "e" in params)) {
        error("Link Corrupted: The link is missing necessary parameters. Please double-check the source link.");
        return;
    }

    // Check that the version in the parameters is valid
    if (!(params["v"] in apiVersions)) {
        error("Unsupported Link Version: This link uses an unsupported security protocol. It may be corrupted.");
        return;
    }
